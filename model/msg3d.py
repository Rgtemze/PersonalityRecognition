...
class Model(nn.Module):
    def __init__(self,
        ...
        self.data_bn_x = nn.BatchNorm1d(num_person * in_channels * num_point)
        # self.data_bn_laban = nn.BatchNorm1d(num_person * in_channels * num_point)
        ...
        self.l_gcn3d1 = MultiWindow_MS_G3D(in_channels, c1, A_binary, num_g3d_scales, window_stride=1)
    ...
    def forward(self, x):
        ...
        # laban = laban[:, 3:, ...]
        #
        # laban = laban.permute(0, 4, 3, 1, 2).contiguous().view(N, M * V * C, T)
        # laban = self.data_bn_laban(laban)
        # laban = laban.view(N * M, V, C, T).permute(0,2,3,1).contiguous()
        #
        # laban_flow = laban
        ...
        # laban_flow = F.relu(self.l_sgcn1(laban_flow) + self.l_gcn3d1(laban_flow), inplace=True)
        # laban_flow = self.l_tcn1(laban_flow)

        # Apply activation to the sum of the pathways
        x = F.relu(self.sgcn1(x) + self.gcn3d1(x), inplace=True)
        x = self.tcn1(x)
        # x = torch.cat((x, laban_flow), 1)
