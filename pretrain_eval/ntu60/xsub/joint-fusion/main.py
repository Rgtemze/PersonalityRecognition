...
ses_df = pd.read_csv("D:/ziya/st-gcn/UDIVA/metadata_train/sessions_train.csv", dtype={'ID': str})
ppl_df = pd.read_csv("D:/ziya/st-gcn/UDIVA/metadata_train/parts_train.csv")
number_of_people = len(ppl_df)
print('number_of_people', number_of_people)
print('number_of_sessions', len(ses_df))
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        self.train_writer.add_scalar('loss', mean_loss, epoch)
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                if True:#self.arg.phase == 'test':
                    people_ids = ppl_df['ID'].to_numpy()
                    predictions = {people_id: np.zeros((5,)) for people_id in people_ids}
                    ...
                    for i, data in enumerate(pred):
                        ...
                        predictions[person_id] += pred[i]
                        # print('---')
                    predictions_data = []
                    gt_data = []
                    for person_id, count in people_occurence_counts.items():
                        if count == 0: continue
                        prediction = predictions[person_id] / people_occurence_counts[person_id]
                        gt = ppl_df[ppl_df['ID'] == person_id][OCEAN_COLUMNS].to_numpy()[0]
                    ...
                    # predictions_df.to_csv("./personality_recognition/input/res/predictions.csv", index=False)
                    # gt_df.to_csv("./personality_recognition/input/ref/ground_truth.csv", index=False)
