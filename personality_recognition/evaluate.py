...
def evalauate(input_path):
    ...
    if len(gt_dict) != len(predictions_dict):
        raise Exception("Different number of participants between ground truth (" + str(
            len(gt_dict)) + ") and predictions files (" + str(len(predictions_dict)) + ").")
    for participant_id in gt_dict.keys():
...
def main():
    ...
    try:
        ...
        if exception.errno != errno.EEXIST:
            raise
    mse_per_class = evalauate(sys.argv[1])
